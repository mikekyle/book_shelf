...
3. Read [`BOOK_CORE_LIVE.md`](BOOK_CORE_LIVE.md) for the deployed base URL (port 8002), same-origin shelf (port 8003), and integration quirks.
...
- Deploy: GitHub Pages (+ Oracle same-origin demo until HTTPS is ready)

## Config

Public `VITE_BOOK_CORE_API_BASE` (or equivalent) pointing at the deployed `book_core` base URL.
Use `same-origin` when the SPA is served behind a proxy that forwards `/api`.
