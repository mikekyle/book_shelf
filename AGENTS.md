...
- Contract drift to watch in live mode: `/api/v1/projections/meta` returns `layouts` as an array of strings plus `pca_variance_explained` (12 floats) and `book_count`, NOT the `{mode,label,variance}[]` shape in `src/types.ts`/`api-contracts.md`. `visual-shelf` and `books/{id}` match. Add an adapter before relying on live projection meta for the layout-toggle labels/variance badge.
